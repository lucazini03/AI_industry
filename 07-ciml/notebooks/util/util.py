...
class CstRULRegressor(keras.Model):
    def __init__(self, rul_pred, alpha, beta, maxrul, dual_ascent=False):
        super(CstRULRegressor, self).__init__()
        # Memorize whether dual ascent should be used
        self.dual_ascent = dual_ascent
    ...
    def compile(self, *pargs, **nargs):
        ...
        if self.dual_ascent:
            self.dual_optimizer.build([self.beta])

    def call(self, data):
        return self.rul_pred(data)

    def _custom_loss(self, x, y_true, idx, flags, sign=1):
        ...
        if self.dual_ascent:
            loss = self.alpha * mse + tf.math.minimum(self.beta, self.max_beta) * cst
        else:
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            loss, mse, cst = self._custom_loss(x, y_true, idx, flags, sign=1)
        ...
        if self.dual_ascent:
            ...
            self.dual_optimizer.apply_gradients(zip([grads], [self.beta]))
